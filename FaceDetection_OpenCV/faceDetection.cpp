...
int main() {
    ...
    cv::Mat image = cv::imread("SSubedi.jpg");  // Change "SSubedi.jpg" to your image path
    ...
    std::vector<cv::Rect> face_det;
    face_cascade.detectMultiScale(img_gray, face_det);

    // Draw rectangles around detected faces
    for (size_t i = 0; i < face_det.size(); i++) {
        cv::rectangle(image, face_det[i], cv::Scalar(0, 0, 255), 5);  // Blue rectangles around faces
    }
